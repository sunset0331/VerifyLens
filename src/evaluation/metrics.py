...
class BenchmarkMetrics:
    ...
    def record_sample(self, doc_type: str, ground_truth: Dict[str, Any], predicted: Dict[str, Any], json_valid: bool, latency_ms: float, parse_error: str = None):
        ...
        if doc_type not in self.doc_types:
            self.doc_types[doc_type] = {"total": 0, "exact_match": 0, "fields": {}}
        ...
        if not hasattr(self, "exact_match_no_doctype_count"):
            self.exact_match_no_doctype_count = 0
            self.core_identity_match_count = 0
            for dt in self.doc_types:
                self.doc_types[dt]["exact_match_no_doctype"] = 0
                self.doc_types[dt]["core_identity_match"] = 0
        
        if "exact_match_no_doctype" not in self.doc_types[doc_type]:
             self.doc_types[doc_type]["exact_match_no_doctype"] = 0
             self.doc_types[doc_type]["core_identity_match"] = 0
    ...
    def compute_summary(self) -> Dict[str, Any]:
        ...
        if sorted_latencies:
            ...
            latency_stats["p95"] = round(sorted_latencies[idx_p95], 1) if idx_p95 < len(sorted_latencies) else latency_stats["max"]
        
        field_accuracies = {}
        for f, stats in self.fields.items():
            field_accuracies[f] = round((stats["correct"] / stats["total"]) * 100, 2) if stats["total"] > 0 else 0
            
        doc_type_stats = {}
        for dt, dt_stats in self.doc_types.items():
            dt_field_acc = {}
            for f, stats in dt_stats["fields"].items():
                dt_field_acc[f] = round((stats["correct"] / stats["total"]) * 100, 2) if stats["total"] > 0 else 0
                
            doc_type_stats[dt] = {
                "samples": dt_stats["total"],
                "exact_match_rate": round((dt_stats["exact_match"] / dt_stats["total"]) * 100, 2) if dt_stats["total"] > 0 else 0,
                "exact_match_no_doctype_rate": round((dt_stats.get("exact_match_no_doctype", 0) / dt_stats["total"]) * 100, 2) if dt_stats["total"] > 0 else 0,
                "core_identity_exact_match_rate": round((dt_stats.get("core_identity_match", 0) / dt_stats["total"]) * 100, 2) if dt_stats["total"] > 0 else 0,
                "field_accuracies": dt_field_acc
            }

        return {
            "total_samples": self.total_samples,
            "json_valid_rate": round((self.json_valid_count / self.total_samples) * 100, 2),
            "exact_match_rate": round((self.exact_match_count / self.total_samples) * 100, 2),
            "exact_match_no_doctype_rate": round((getattr(self, "exact_match_no_doctype_count", 0) / self.total_samples) * 100, 2) if self.total_samples > 0 else 0,
            "core_identity_exact_match_rate": round((getattr(self, "core_identity_match_count", 0) / self.total_samples) * 100, 2) if self.total_samples > 0 else 0,
